...
namespace Primavera.Invoice
{
    public partial class frmInvoice : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            sql.Append("select c.Cliente, c.nome, COUNT(cd.Id) Total from Clientes C with (nolock)");
            sql.Append("Inner join CabecDoc CD with (nolock) ON c.Cliente=CD.Entidade group by c.Cliente, c.nome");

            txtEntidade.Text = PriEngine.Platform.Listas.GetF4SQL("Clientes", sql.ToString() , "cliente,nome");
        }
    }
}
